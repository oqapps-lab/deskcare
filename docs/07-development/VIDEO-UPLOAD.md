...
**Кому:** Russell Zain
**Что:** Залить **58 готовых видео-атомов** (из 64 запланированных) в Supabase Storage, чтобы плеер в приложении начал показывать их вместо stick-figure placeholder'ов.
**Сколько займёт:** один проход через `tsx`-скрипт, ~5–15 минут на upload в зависимости от твоего интернета.

**Статус по доке `docs/05-database/EXERCISES-LIBRARY-SUPABASE.md` (последняя версия Russell'а):**
- ✅ **58 атомов сняты** — заливаем сейчас.
- ⏳ **6 в очереди**, к ним вернёмся позже: `W3` (wrist-circles), `W6` (reverse-prayer), `W7` (finger-fan), `W8` (median-nerve-glide), `W9` (thumb-stretch), `W10` (finger-wave).

В таблице §3 ниже эти 6 строк помечены `⏳ pending` — пропускаем их в текущей загрузке, плеер для этих атомов покажет stick-figure placeholder до момента, когда они появятся. Скрипт обрабатывает пропуски без ошибок.
...
| W3   | ⏳ pending                       | Круги запястьями                  |
| W4   | `fist-open-close.mp4`           | Сжатие/раскрытие кулаков          |
| W5   | `prayer-stretch.mp4`            | «Молитвенный» растяжитель         |
| W6   | ⏳ pending                       | Обратный «молитвенный»            |
| W7   | ⏳ pending                       | Поочерёдный веер пальцами         |
| W8   | ⏳ pending                       | Скольжение срединного нерва       |
| W9   | ⏳ pending                       | Растяжка большого пальца          |
| W10  | ⏳ pending                       | «Волна» пальцами                  |
...
**Сейчас грузим: 58 файлов.** Оставшиеся 6 (помечены ⏳ pending) — заливаются отдельным проходом, когда сняты.
